...
export default function AdminPage(){
  ...
  const uploadPlayerFace=async(file?:File)=>{
    if(!file)return;
    const form=new FormData();form.append('file',file);
    const response=await fetch(`${api}/admin/uploads/player-face`,{method:'POST',credentials:'include',headers:{'x-admin-key':key},body:form});
    const data=await response.json();
    if(!response.ok)throw new Error(Array.isArray(data.message)?data.message.join(', '):data.message);
    setPlayerPhotoUrl(data.url);setNotice('Player face uploaded');
  };
  ...
  return <main className="page-shell admin-shell">
    ...
    <section className="admin-section"><span className="eyebrow">SQUADS</span><div className="admin-grid">
      ...
      <AdminCard icon={<UserPlus/>} step="06" title="Create player"><form onSubmit={e=>handle(e,async f=>{await post('/admin/players',{legalName:f.get('legalName'),knownAs:f.get('knownAs')||undefined,dateOfBirth:f.get('dateOfBirth')?new Date(`${f.get('dateOfBirth')}T00:00:00`).toISOString():undefined,nationalityCode:f.get('nationalityCode')||'ZW',preferredFoot:f.get('preferredFoot')||undefined,photoUrl:playerPhotoUrl||undefined,position:f.get('position')});setPlayerPhotoUrl('')},'Player created')}><label>Legal name<input name="legalName" required/></label><label>Known as<input name="knownAs"/></label><label>Date of birth<input name="dateOfBirth" type="date"/></label><label>Nationality<input name="nationalityCode" defaultValue="ZW" maxLength={2}/></label><label>Preferred foot<select name="preferredFoot" defaultValue=""><option value="">Unknown</option><option>LEFT</option><option>RIGHT</option><option>BOTH</option></select></label><label>Position<input name="position" required placeholder="FW"/></label><label className="cover-upload"><ImageUp/><span>Upload player face</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>uploadPlayerFace(e.target.files?.[0]).catch(reason=>setError(reason.message))}/><small>Validated JPG, PNG or WebP · maximum 5 MB</small></label><label>Face URL<input value={playerPhotoUrl} onChange={e=>setPlayerPhotoUrl(e.target.value)} type="url" placeholder="Uploaded URL appears here"/></label><button>Create player</button></form></AdminCard>
