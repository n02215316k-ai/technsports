...
type Result={status:string;confidence?:number;agreeingSources?:number;observationId?:string;identityStatus?:string;playerId?:string;subjectId?:string;queued?:boolean;queueId?:string;eventType?:string;team?:string;playerName?:string;createdAt?:number;clientEventId?:string};
...
export default function CollectorConsole(){
  ...
  const pushRecent=(item:Result)=>setRecent(current=>{const key=item.clientEventId;if(key&&current.some(entry=>entry.clientEventId===key))return current.map(entry=>entry.clientEventId===key?{...entry,...item}:entry).slice(0,8);return[item,...current].slice(0,8)});
  const submitObservation=async(options?:{eventType?:string;team?:string;playerId?:string;clientEventId?:string;quick?:boolean})=>{if(!user||!authorised){setError('Sign in with a collector account before submitting match data.');return}const type=options?.eventType??eventType,teamValue=options?.team??team,subjectPlayer=options?.playerId??playerId;const typeRequiresPlayer=playerEvents.includes(type);if(typeRequiresPlayer&&!subjectPlayer){setError('Choose a player before recording this event.');return}const clientEventId=options?.clientEventId??`${user.id}-${id}-${eventKey}`;if(options?.quick)setQuickInFlight(count=>count+1);else setSending(true);setError('');const pending:Result={status:'SENDING',eventType:type,team:teamValue,playerName:playerName(subjectPlayer),createdAt:Date.now(),clientEventId};setResult(pending);pushRecent(pending);try{const identity=typeRequiresPlayer?(subjectPlayer==='__unlisted__'?{unlistedPlayerName:unlistedName,teamId:teamValue}:{playerId:subjectPlayer}):{};const response=await submitOrQueue(`${api}/matches/${id}/observations`,{contributorId:user.id,eventType:type,matchSecond:minute*60,payload:payloadFor(type,teamValue),clientEventId,...identity});const complete={...(response as Result),eventType:type,team:teamValue,playerName:playerName(subjectPlayer),createdAt:Date.now(),clientEventId};setResult(complete);pushRecent(complete)}catch(reason){const failed:Result={status:'FAILED',eventType:type,team:teamValue,playerName:playerName(subjectPlayer),createdAt:Date.now(),clientEventId};setResult(failed);pushRecent(failed);setError(reason instanceof Error?reason.message:'Submission failed')}finally{if(options?.quick)setQuickInFlight(count=>Math.max(0,count-1));else setSending(false)}};
